...
<?php
   ...
   // $_SESSION['couns']=$name1['name'];
    ...
    if($access1['access_level']==4)
        ...
        $res=mysqli_query($connect1,"SELECT * FROM `{$name1['name']}` where dat='$dt'");
    ...
    if($count!=0)
    {
        ...
        for($i=0;$i<$count;$i++)
        {
            ...
            echo "<td><a id='accept' href='http://localhost/hostel%20project/classteacher.php?roll=$regd'>Accept</a></td><td><a id='reject'>Reject</a></td></tr>";
        }
        echo "</table>";
    }
    else
